<template>
  <div class="truck-detail">
    ...
    <vs-input v-if="truck.truckId" class="w-full mb-6" v-model="owner.name" disabled label-placeholder="Chủ xe" />
    <vs-input v-if="truck.truckId" class="w-full mb-6" disabled v-model="owner.phone" label-placeholder="Số điện thoại chủ xe" />
    <vs-row>
      <vs-col vs-w="6">
        <vs-select v-if="!truck.truckId" placeholder="Chủ xe" label="Chủ xe" v-model="truck.customerId" class="mb-4">
          <vs-select-item :key="index" :value="item.id" :text="item.name" v-for="(item, index) in customers" />
        </vs-select>
      </vs-col>
      <vs-col vs-w="6">
