<template>
  <div class="post-form mb-2">
    <div class="px-10 pt-4 pb-16 bg-gray-100 rounded-lg">
      ...
      <vs-row>
        <vs-col vs-w="6">
          <vs-select
            ...
            class=""
            label="Xe được chọn"
            v-model="post.truckId"
          >
            <vs-select-item
              :key="index"
              :value="item.truck_id"
              :text="item.license_plates"
      ...
      <vs-row>
        <vs-col vs-w="6">
          <vs-input placeholder="VD: 15 tấn" class="" label="Trọng tải hàng" v-model="post.weightProduct" />
        </vs-col>
        <vs-col vs-w="6">
          <vs-select
            v-if="post.postId"
            placeholder="Loại hàng nhận"
            class=""
            label="Loại hàng yêu cầu"
            v-model="post.postItemType"
            multiple
          >
            <vs-select-item
              :key="index"
    ...
    <div class="mt-4 flex justify-end">
      ...
      <vs-button color="danger" icon="delete" v-if="post.postId" class="mx-4" @click="$emit('actionDelete')">
        Xoá
      </vs-button>
